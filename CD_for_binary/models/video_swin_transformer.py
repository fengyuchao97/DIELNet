...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        # self.poisson = encoding.PoissonEncoder(step_mode='m')
    ...
    #     def _interpolate(im1, im2, rate_map):
    ...
    #     frames = _interpolate(im1, im2, rate_map)
    #     return frames
    
    def forward(self, x):
        """Forward function."""

        # x = self.pair_to_video(input1, input2)
        # # x = rearrange(x, 'n d c h w -> c n d h w')
        # # x = self.poisson(x)
        ...
        for layer in self.layers:
            output = rearrange(x, 'n c d h w -> n (c d) h w')
            stage_out.append(output)
