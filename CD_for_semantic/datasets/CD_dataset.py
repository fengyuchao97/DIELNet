...
def swapFrequencies_3Channel(Images1, Images2, r):
    ...
    return np.transpose(tmp_X_new, (1, 2, 0)).astype(np.uint8), np.transpose(tmp_Y_new, (1, 2, 0)).astype(np.uint8)
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(L_path):
            ...
            # print(f"Label path {L_path} does not exist.")
            
            [img_weak, img_B_weak], [label_binary] = self.augm.transform([img, img_B],[label_binary], to_tensor=self.to_tensor)
            compute_semantic = False
            output = torch.zeros(size=(1, 256, 256), dtype=torch.float32).to(img_weak.device).long()
        ...
        return {'name': name, 'A': img_weak, 'B': img_B_weak, 'L': output, 'L_binary': label_binary, 'aug': img_aug, 'compute_semantic': compute_semantic} 
