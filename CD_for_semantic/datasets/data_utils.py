...
class CDDataAugmentation: 
    ...
    def transform(self, imgs, labels, to_tensor=True):
        ...
        if to_tensor:
            imgs, labels = self.to_tensor(imgs, labels)
            
        return imgs, labels
    
    def to_tensor(self, imgs, labels):
    ...
    def transform_strong(self, imgs, labels, to_tensor=True):
        imgs = [TF.to_pil_image(img) for img in imgs]
        # imgs = [transforms.ColorJitter(0.2, 0.2, 0.2, 0.2)(img) for img in imgs]

        radius=random.uniform(0.1,0.2)
        imgs = [transforms.ColorJitter(radius, radius, radius, radius)(img) for img in imgs]

        # [TF.adjust_hue(TF.adjust_saturation(TF.adjust_contrast(TF.adjust_brightness(img,0.5),0.5),0.5),0.25) for img in imgs]
        # transforms.ColorJitter(0.5, 0.5, 0.5, 0.25)(img) 
        imgs = [img.filter(ImageFilter.GaussianBlur(radius=random.uniform(0.1, 2.0))) for img in imgs]
        if to_tensor:
            imgs, _ = self.to_tensor(imgs, [])
            
        return imgs
    
    def transform_strong2(self, imgs, labels, to_tensor=True):
        imgs = [TF.to_pil_image(img) for img in imgs]
        # imgs = [transforms.ColorJitter(0.4, 0.4, 0.4, 0.4)(img) for img in imgs]
        radius=random.uniform(0.1,0.2)
        imgs = [transforms.ColorJitter(radius, radius, radius, radius)(img) for img in imgs]
    ...
    def transform_strong3(self, imgs, labels, to_tensor=True):
        imgs = [TF.to_pil_image(img) for img in imgs]
        # imgs = [transforms.ColorJitter(0.1, 0.1, 0.1, 0.1)(img) for img in imgs]
        radius=random.uniform(0.1,0.2)
        imgs = [transforms.ColorJitter(radius, radius, radius, radius)(img) for img in imgs]
