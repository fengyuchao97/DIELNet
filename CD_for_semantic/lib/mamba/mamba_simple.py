...
try:
    from lib.mamba.ops.selective_scan_interface import selective_scan_fn, mamba_inner_fn, bimamba_inner_fn, mamba_inner_fn_no_out_proj
except ImportError:
    selective_scan_fn, mamba_inner_fn, bimamba_inner_fn, mamba_inner_fn_no_out_proj = None, None, None, None, None

try:
    from lib.mamba.ops.triton.selective_state_update import selective_state_update
except ImportError:
    selective_state_update = None

try:
    from lib.mamba.ops.triton.layernorm import RMSNorm, layer_norm_fn, rms_norm_fn
except ImportError:
    ...
    def forward(self, hidden_states, inference_params=None,extra_emb=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                ...
                # F.linear(rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias)
                if not self.if_devide_out:
                    out_x = F.linear(rearrange(out, "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
                    out_y = F.linear(rearrange(out_b, "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
