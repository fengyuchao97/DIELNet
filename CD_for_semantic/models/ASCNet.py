...
# class ECCVNet(nn.Module): # only pair
#     def __init__(self, num_classes=1, normal_init=True, pretrained=False):
#         super(ECCVNet, self).__init__()
...
#         self.global_consrative2 = ContrastiveAtt_Block(192, chunk_number=16) # Local_Block(192)# 
#         self.global_consrative1 = ContrastiveAtt_Block(80, chunk_number=16) # Local_Block(80)# 
...
#         dec1,output_middle2 = self.decoder1(fuse2,fuse3) # 64+64 + 96+128
#         dec2,output_middle1 = self.decoder2(fuse1,dec1) # 24+32 + 128
#         dec3,output = self.decoder3(fuse0,dec2) # 16 +
...
#             if labels is not None:
#                 return output, output_middle1, output_middle2, loss_att
#             else:
#                 return output, output_middle1, output_middle2
#         else:
#             output = F.interpolate(output, size=img1.shape[2:])
...
# class ECCVNet(nn.Module): # only video
#     def __init__(self, num_classes=1, normal_init=True, pretrained=False):
#         super(ECCVNet, self).__init__()
...
#         self.decoder1 = DecBlock(128, 128, num_classes) 
#         self.decoder2 = DecBlock(160, 64, num_classes)
...
#             out3, loss_att_swim3 = self.fam32_1(torch.cat([encoder_outputs[1], self.upsamplex2(out2)],1),labels) #64
#             out4, loss_att_swim4 = self.fam43_1(torch.cat([encoder_outputs[0], self.upsamplex2(self.Translayer3_1(out3))],1),labels) #32
#             loss_att = loss_att_swim3 + loss_att_swim4
#         else:
#             out2 = self.Translayer2_1(encoder_outputs[2]) #64
#             out3 = self.fam32_1(torch.cat([encoder_outputs[1], self.upsamplex2(out2)],1)) #64
#             out4 = self.fam43_1(torch.cat([encoder_outputs[0], self.upsamplex2(self.Translayer3_1(out3))],1)) #32
...
#         dec1,output_middle = self.decoder1(out3,out2) # 64+64 + 96+128
#         dec2,output = self.decoder2(out4,dec1) # 24+32 + 128

#         if return_aux:
#             output_middle = F.interpolate(output_middle, size=output.shape[2:])
#             output = F.interpolate(output, size=img1.shape[2:])
            
#             pred_v = self.conv_out_v(self.upsample_pixel(out4))
#             pred_v = F.interpolate(pred_v, size=output.shape[2:])
#             output = torch.sigmoid(output)
#             output_middle = torch.sigmoid(output_middle)
    
#             pred_v = torch.sigmoid(pred_v)

#             if labels is not None:
#                 return output, output_middle, pred_v, loss_att
#             else:
#                 return output, output_middle, pred_v,
#         else:
#             output = F.interpolate(output, size=img1.shape[2:])
#             output = torch.sigmoid(output)
#             if labels is not None:
...
#     def __init__(self, in_channels, block_size=3, num_samples=8):
#         super(PixelwiseAttention, self).__init__()
#         self.block_size = block_size
#         self.num_samples = num_samples
#         self.in_channels = in_channels
#         self.out_channels = in_channels  # 保持输入输出通道数一致
#         self.scale = (in_channels * (block_size ** 2)) ** -0.5  # 缩放因子

#     def forward(self, T1, T2):
#         B, C, H, W = T1.size()
#         # 提取所有重叠的小块
#         padding = self.block_size // 2
#         T1_padded = F.pad(T1, (padding, padding, padding, padding))
#         T2_padded = F.pad(T2, (padding, padding, padding, padding))
        
#         # 初始化注意力矩阵
#         attention_matrix = torch.zeros((B, H*W, H*W), device=T1.device)
#         attention_matrix_others = torch.zeros((B, H*W, H*W), device=T1.device)

#         # 遍历所有像素点，对每个像素点计算注意力
#         for i in range(H):
#             for j in range(W):
#                 block1 = T1_padded[:, :, i:i+self.block_size, j:j+self.block_size]
#                 block2 = T2_padded[:, :, i:i+self.block_size, j:j+self.block_size]
#                 block1_flat = block1.reshape(B, -1, self.block_size*self.block_size)
#                 block2_flat = block2.reshape(B, -1, self.block_size*self.block_size)
                
#                 # 计算点积注意力
#                 attn_scores = torch.bmm(block1_flat.permute(0, 2, 1), block2_flat) * self.scale
#                 attn_scores = torch.sum(attn_scores, dim=(1, 2), keepdim=False)
#                 attention_matrix[:, i*W+j, i*W+j] = attn_scores

#                 center_block_T1 = T1_padded[:, :, i:i+self.block_size, j:j+self.block_size].reshape(B, -1, self.block_size*self.block_size)
#                 center_block_T2 = T2_padded[:, :, i:i+self.block_size, j:j+self.block_size].reshape(B, -1, self.block_size*self.block_size)
#                 rand_indices = get_random_indices(self.num_samples, H, W, except_indices=(i, j))
                
#                 for r_i, r_j in rand_indices:
#                     rand_block_T1 = T1_padded[:, :, r_i:r_i+self.block_size, r_j:r_j+self.block_size].reshape(B, -1, self.block_size*self.block_size)
#                     rand_block_T2 = T2_padded[:, :, r_i:r_i+self.block_size, r_j:r_j+self.block_size].reshape(B, -1, self.block_size*self.block_size)
                    
#                     # Compute attention scores
#                     #  torch.bmm(query.permute(0, 2, 1), key) * self.scale
#                     attn_scores_A1 = torch.bmm(center_block_T1.permute(0, 2, 1), rand_block_T1) * self.scale
#                     attn_scores_A2 = torch.bmm(center_block_T1.permute(0, 2, 1), rand_block_T2) * self.scale
#                     attn_scores_A3 = torch.bmm(center_block_T2.permute(0, 2, 1), rand_block_T1) * self.scale
#                     attn_scores_A4 = torch.bmm(center_block_T2.permute(0, 2, 1), rand_block_T2) * self.scale
#                     attn_scores_four = attn_scores_A1 - attn_scores_A2 + attn_scores_A3 - attn_scores_A4
#                     attn_scores_four = torch.sum(attn_scores_four, dim=(1, 2), keepdim=False)
#                     # Update the final attention matrix based on the formula
#                     attention_matrix_others[:, i*W+j, r_i*W+r_j] = (attention_matrix_others[:, i*W+j, r_i*W+r_j] + attn_scores_four) / 2
                    
#         # Apply absolute value and sigmoid
#         attention_matrix_others = torch.abs(attention_matrix_others)
        
#         # 应用注意力分布
#         attention = F.sigmoid(attention_matrix+attention_matrix_others)  # 应用sigmoid激活
#         T1_output = torch.bmm(attention, T1.view(B, C, H*W).permute(0, 2, 1))
#         T2_output = torch.bmm(attention, T2.view(B, C, H*W).permute(0, 2, 1))
        
#         # 重新调整形状以匹配原始输入
#         T1_output = T1_output.permute(0, 2, 1).view(B, C, H, W)
#         T2_output = T2_output.permute(0, 2, 1).view(B, C, H, W)
        
#         return T1_output, T2_output

# class PixelwiseAttention(nn.Module):
#     def __init__(self, in_channels, block_size=1, num_samples=8, key_dim=16, num_heads=8, attn_ratio=2, norm_cfg=dict(type='BN2d', requires_grad=True), ):
#         super(PixelwiseAttention, self).__init__()
#         self.block_size = block_size
#         self.num_samples = num_samples
#         self.in_channels = in_channels
...
#         self.to_q = Conv2d_BN(in_channels, self.out_channels, 1, norm_cfg=norm_cfg)
#         self.to_k = Conv2d_BN(in_channels, self.out_channels, 1, norm_cfg=norm_cfg)
#         self.to_v_1 = Conv2d_BN(in_channels, self.dh, 1, norm_cfg=norm_cfg)
#         self.to_v_2 = Conv2d_BN(in_channels, self.dh, 1, norm_cfg=norm_cfg)
...
#         q = self.to_q(T1_padded)
#         k = self.to_k(T2_padded)
        
#         # 初始化注意力矩阵
#         attention_matrix = torch.zeros((B, H*W, H*W), device=T1.device)
#         # attention_matrix_others = torch.zeros((B, self.num_heads, H*W, H*W), device=T1.device)

#         # 遍历所有像素点，对每个像素点计算注意力
#         for i in range(H):
#             for j in range(W):
#                 block1 = q[:, :, i:i+self.block_size, j:j+self.block_size]
#                 block2 = k[:, :, i:i+self.block_size, j:j+self.block_size]
#                 block1_flat = block1.reshape(B, -1, self.block_size*self.block_size)
#                 block2_flat = block2.reshape(B, -1, self.block_size*self.block_size)
                
#                 # 计算点积注意力
#                 attn_scores = torch.matmul(block1_flat.permute(0, 2, 1), block2_flat) * self.scale
#                 attn_scores = torch.sum(attn_scores, dim=(1, 2), keepdim=False)
#                 attention_matrix[:, i*W+j, i*W+j] = attn_scores

#         #         center_block_T1 = q[:, :, i:i+self.block_size, j:j+self.block_size].reshape(B, self.num_heads, -1, self.block_size*self.block_size)
#         #         center_block_T2 = k[:, :, i:i+self.block_size, j:j+self.block_size].reshape(B, self.num_heads, -1, self.block_size*self.block_size)
#         #         rand_indices = get_random_indices(self.num_samples, H, W, except_indices=(i, j))
                
#         #         for r_i, r_j in rand_indices:
#         #             rand_block_T1 = q[:, :, r_i:r_i+self.block_size, r_j:r_j+self.block_size].reshape(B, self.num_heads, -1, self.block_size*self.block_size)
#         #             rand_block_T2 = k[:, :, r_i:r_i+self.block_size, r_j:r_j+self.block_size].reshape(B, self.num_heads, -1, self.block_size*self.block_size)
                    
#         #             # Compute attention scores
#         #             #  torch.bmm(query.permute(0, 2, 1), key) * self.scale
#         #             attn_scores_A1 = torch.matmul(center_block_T1.permute(0, 1, 3, 2), rand_block_T1) * self.scale
#         #             attn_scores_A2 = torch.matmul(center_block_T1.permute(0, 1, 3, 2), rand_block_T2) * self.scale
#         #             attn_scores_A3 = torch.matmul(center_block_T2.permute(0, 1, 3, 2), rand_block_T1) * self.scale
#         #             attn_scores_A4 = torch.matmul(center_block_T2.permute(0, 1, 3, 2), rand_block_T2) * self.scale
#         #             attn_scores_four = attn_scores_A1 - attn_scores_A2 + attn_scores_A3 - attn_scores_A4
#         #             attn_scores_four = torch.sum(attn_scores_four, dim=(2, 3), keepdim=False)
#         #             # Update the final attention matrix based on the formula
#         #             attention_matrix_others[:, :, i*W+j, r_i*W+r_j] = (attention_matrix_others[:, i*W+j, r_i*W+r_j] + attn_scores_four) / 2
...
#         # 应用注意力分布
#         attention = F.sigmoid(attention_matrix)  # 应用sigmoid激活
#         T1_output = torch.matmul(attention, T1.view(B, C, H*W).permute(0, 2, 1))
#         T2_output = torch.matmul(attention, T2.view(B, C, H*W).permute(0, 2, 1))
...
class HybridTokenMixer(nn.Module): ### D-Mixer
    ...
    def forward(self, x1, x2, relative_pos_enc=None):
        ...
        x_out1_2, x_out2_2, _ = self.global_unit(x1_high, x2_high)
...
class ASCNet_multi_granularity(nn.Module): # 1020
    def __init__(self, num_classes=3, normal_init=True, pretrained=False):
        super(ASCNet_multi_granularity, self).__init__()
    ...
    def forward(self, imgs, labels=None, return_aux=True):
        ...
        if return_aux:
            ...
            # output_middle1 = F.softmax(output_middle1, dim=1) # torch.sigmoid(output_middle1)
            # output_middle2 = F.softmax(output_middle2, dim=1) # torch.sigmoid(output_middle2)
    
            # pred_v = F.softmax(pred_v, dim=1) # torch.sigmoid(pred_v)
