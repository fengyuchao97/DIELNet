...
class CVPRNet(nn.Module): # 1020
    def __init__(self, num_classes=1, normal_init=True, pretrained=False):
        ...
        self.global_consrative1 = ContrastiveAtt_Block(80, chunk_number=32) # Local_Block(80)# 
        self.local_consrative2 = Local_Block(48, window_size=16, chunk_number=64)
        self.local_consrative1 = Local_Block(32, window_size=16, chunk_number=64)
        ...
        self.fam32_1 = Align(112, 64, chunk_number=32) #DRAtt(112,64) # SimpleResBlock DRAtt(112,64) # 
        self.Translayer3_1 = BasicConv2d(64,32,1)
        self.fam43_1 = Align(56, 32, chunk_number=64) # DRAtt(56,32) DRAtt(56,32) # 
    ...
    def forward(self, imgs, labels=None, return_aux=True):
        ...
        if labels is not None:
            ...
            cur1_0, cur2_0, loss_local_att1, loss_local_res1 = self.local_consrative1(torch.cat([c0,c1],1), torch.cat([c0_img2,c1_img2],1),labels) # 16, 128 , 128 -> 8, 128 , 128
            cur1_1, cur2_1, loss_local_att2, loss_local_res2 = self.local_consrative2(torch.cat([c2,c3],1), torch.cat([c2_img2,c3_img2],1),labels) # 48, 64 , 64 -> 24, 64 , 64
            cur1_2, cur2_2, loss_att2, loss_res2 = self.global_consrative1(torch.cat([c4,c5],1), torch.cat([c4_img2,c5_img2],1),labels) # 80, 32 , 32 -> 32, 32 , 32
            cur1_3, cur2_3, loss_att1, loss_res1 = self.global_consrative2(torch.cat([c6,c7],1), torch.cat([c6_img2,c7_img2],1),labels) # 192, 16 , 16 -> 64, 16 , 16
...
# class CVPRNet(nn.Module): # only pair
#     def __init__(self, num_classes=1, normal_init=True, pretrained=False):
#         super(CVPRNet, self).__init__()
...
# class CVPRNet(nn.Module): # only video
#     def __init__(self, num_classes=1, normal_init=True, pretrained=False):
#         super(CVPRNet, self).__init__()
