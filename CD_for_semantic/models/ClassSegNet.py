...
class ClassSegNet(nn.Module):
    def __init__(self, num_classes=2, drop_rate=0.2, normal_init=True, pretrained=False, show_Feature_Maps=False, pooling="gmp"):
        ...
        self.sub_fc1 = nn.Linear(in_features=256, out_features=64, bias=True)
        self.sub_fc2 = nn.Linear(in_features=64, out_features=1, bias=True)

        self.cat_fc1 = nn.Linear(in_features=256, out_features=64, bias=True)
        self.cat_fc2 = nn.Linear(in_features=64, out_features=1, bias=True)
    ...
    def forward(self, imgs1, imgs2, labels=None):
        batch, channel, height, wight = imgs1.shape
        ...
        # c4 = self.resnet.avgpool(self.resnet.layer4(c3))
        ...
        # c4_img2 = self.resnet.avgpool(self.resnet.layer4(c3_img2))
        ...
        class_predict1 = self.sigmoid(self.cat_fc2(self.cat_fc1(self.pooling(cross_result2,(1,1)).view(-1, 256))))
        # class_predict2 = self.sigmoid(self.sub_fc2(self.sub_fc1(self.pooling(torch.abs(cur1_2-cur2_2),(1,1)).view(-1, 256))))

        # class_predict = (class_predict1 + class_predict2)/2

        for i in range(batch):
            if class_predict1[i,:] < 0.2: #or class_predict2[i,:] < 0.2 :
                out[i,:,:,:] = 0
                out_2[i,:,:,:] = 0
            # elif class_predict[i,:] > 0.8:
            #     class_predict[i,:] = 1

        return class_predict1, out, out_2 #, out_middle_1, out_middle_2
    ...
    def init_weights(self):
        ...
        self.cat_fc1.apply(init_weights)
        self.sub_fc2.apply(init_weights)
        self.sub_fc1.apply(init_weights)
        self.cat_fc2.apply(init_weights)