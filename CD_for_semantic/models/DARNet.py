...
class CDSA(nn.Module):
    ...
    """
    def __init__(self, in_c, ds=1, mode='CCA'):
        ...
        self.mode = mode
        if self.mode == 'CCA':
            self.Self_Att = CrissCrossAttention(self.in_C)#BAM(self.in_C,ds)
        if self.mode == 'BAM':
            self.Self_Att = BAM(self.in_C,ds)#BAM(self.in_C,ds)
        # elif self.mode == 'PAM':
        #     self.Self_Att = PAM(in_channels=self.in_C, out_channels=self.in_C, sizes=[1,2,4,8],ds=self.ds)
        self.apply(self.weights_init)

    def forward(self, x1, x2, recurrent = 2):
        height = x1.shape[3]
        x = torch.cat((x1, x2), 3)
        at1 = self.Self_Att(x)
        x = self.Self_Att(at1)
