...
class CPAMDec_Mix_large(nn.Module):
    ...
    def forward(self,x1,y1,x2,y2,label=None):
        ...
        if label is not None:
            ...
            loss_res = self.loss_generator(out1*(1-label), out2*(1-label))
...
class ContrastiveAtt_large(nn.Module):
    ...
    def forward(self, x, y, label=None):
        ...
        if label is not None:
            cpam_feat1, cpam_feat2, att, loss_att, loss_res = self.cpam_dec_mix(x,cpam_f_x,y,cpam_f_y,label) 
            return cpam_feat1, cpam_feat2, att, loss_att, loss_res
        else: 
            cpam_feat1, cpam_feat2, att = self.cpam_dec_mix(x,cpam_f_x,y,cpam_f_y) 
            return cpam_feat1, cpam_feat2, att
...
class ContrastiveAtt_Block(nn.Module):
    ...
    def forward(self, t1, t2, labels=None, att=None):
        ...
        if att is not None:
            ...
            att = (att_val).unsqueeze(1)
            t1 =  self.scale * att * t1 + t1
            t2 =  self.scale * att * t2 + t2        
...
class Local_LSKAtt(nn.Module):
    ...
    def forward(self, x1, x2, labels = None):   
        ...
        x1 = x1 * att + x1 #+ self.scale*x1 # (att + self.scale*attn_img1) 
        x2 = x2 * att + x2 #+ self.scale*x2 # (att + self.scale*attn_img2) #att # 
        ...
        if labels is not None:
            ...
            att_pred = torch.where(att > 0.5, torch.ones_like(att), torch.zeros_like(att)).float()
            loss_att = self.loss_generator(att_pred, label) # self.loss_generator  BCEDiceLoss
            loss_res = self.loss_generator(x1*(1-label), x2*(1-label))
            return x1, x2, att, loss_att, loss_res
        else:
            return x1, x2, att
...
class GSINet(nn.Module): 
    def __init__(self, num_classes=1, drop_rate=0., normal_init=True, pretrained=False, show_Feature_Maps=False):
        super(GSINet, self).__init__()
        self.video_len = 8
        self.show_Feature_Maps = show_Feature_Maps
        enc_chs = (16,32)
    ...
    def forward(self, imgs1, imgs2, labels=None, return_aux=True):
        ...
        if labels is not None:
            cur1_3, cur2_3, att_3, loss_att3, loss_res3 = self.global_consrative2(torch.cat([c6,c7],1), torch.cat([c6_img2,c7_img2],1),labels) # 192, 16 , 16 -> 64, 16 , 16
            cur1_2, cur2_2, att_2, loss_att2, loss_res2 = self.global_consrative1(torch.cat([c4,c5],1), torch.cat([c4_img2,c5_img2],1),labels,att_3) # 80, 32 , 32 -> 32, 32 , 32
            cur1_1, cur2_1, att_1, loss_att1, loss_res1 = self.local_consrative2(torch.cat([c2,c3],1), torch.cat([c2_img2,c3_img2],1),labels,att_2) # 48, 64 , 64 -> 24, 64 , 64
            cur1_0, cur2_0, att_0, loss_att0, loss_res0 = self.local_consrative1(torch.cat([c0,c1],1), torch.cat([c0_img2,c1_img2],1),labels,att_1) # 16, 128 , 128 -> 8, 128 , 128
...
class GSINet_encoder(nn.Module): 
    def __init__(self, num_classes=1, drop_rate=0., normal_init=True, pretrained=False, show_Feature_Maps=False):
        super(GSINet_encoder, self).__init__()
...
class FresUNet_mixup(nn.Module):
    """FresUNet segmentation network."""

    def __init__(self, input_nbr=2*3, label_nbr=2):
        """Init FresUNet fields."""
        super(FresUNet_mixup, self).__init__()
        ...
        # Decoding stage 3
        self.decres3_1 = BasicBlock_ss(cur_depth + d4 + 64, planes = cur_depth)
        self.decres3_2 = BasicBlock_us(cur_depth, upsamp=2)
        cur_depth = int(cur_depth/2)
        
        # Decoding stage 2
        self.decres2_1 = BasicBlock_ss(cur_depth + d3 + 32, planes = cur_depth)
        ...
        enc_chs = (16,32)
        self.encoder_v2 = VideoEncoder(in_ch=3, enc_chs=enc_chs, expansion=2)
        enc_chs = tuple(ch*self.encoder_v2.expansion for ch in enc_chs)
        self.convs_video = nn.ModuleList(
            [
                Conv1x1(2*ch, ch, norm=True, act=True)
                for ch in enc_chs
    ...
    def forward(self, x1, x2, labels=None):
        ...
        frames = self.generate_transition_video_tensor(x1, x2)

        feats_v = self.encoder_v2(frames) #.transpose(1,2)
        feats_v.pop(0)
        for i, feat in enumerate(feats_v):
            feats_v[i] = self.convs_video[i](self.tem_aggr(feat))
...
#         pad5 = ReplicationPad2d((0, x53.size(3) - x5d.size(3), 0, x53.size(2) - x5d.size(2)))
        ...
        # x = self.decres3_1(x)
        x = self.decres3_1(torch.cat((x, x4, feats_v[1]), 1))
        x = self.decres3_2(x)
        s3_2 = x.size()
        pad3 = ReplicationPad2d((0, s3_1[3] - s3_2[3], 0, s3_1[2] - s3_2[2]))
        x = pad3(x)
        
        x = self.decres2_1(torch.cat((x, x3, feats_v[0]), 1))
        x = self.decres2_2(x)
        s2_2 = x.size()
