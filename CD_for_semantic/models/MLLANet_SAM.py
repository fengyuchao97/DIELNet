...
class densecat_cat_diff(nn.Module):
    ...
    def forward(self, x, y):
        ...
        y2 = self.conv2(y1)
        y3 = self.conv3(y2+y1)
        out = self.conv_out(x1 + x2 + x3 - y1 - y2 - y3)
        return out
...
class MLLANet_SAM(nn.Module): # 1020
    ...
    def forward(self, imgs, labels=None, return_aux=True):
        ...
        if return_aux:
            ...
            return output, output_middle1, output_middle2, output_binary
        else:
            output = F.interpolate(output, size=img1.shape[2:])
            output = torch.sigmoid(output)
            return output
