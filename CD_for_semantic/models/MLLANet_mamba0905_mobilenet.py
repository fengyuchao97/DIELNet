...
from lib.mamba.mamba_simple import Mamba
...
class HybridTokenMixer(nn.Module): ### D-Mixer
    def __init__(self, 
        ...
        self.global_unit = CrossMamba(dim//2) # MLLABlock(dim//2, chunk_numbers=chunk_numbers) 
    ...
    def forward(self, x1, x2, relative_pos_enc=None):
        ...
        x_out1_2, x_out2_2 = self.global_unit(x1_2, x2_2)
...
class Interaction_multi_granularity(nn.Module):
    def __init__(self, in_channels, drop_path=0.1, kernel_size=3, sr_ratio=1, num_groups=2, num_heads=1, chunk_numbers=[8,16],before_attn_dwconv=3, pre_norm=True):
        ...
        self.attn = Interaction_hybrid_minxer(dim, kernel_size=kernel_size) 
...
class CrossMamba(nn.Module):
    def __init__(self, dim):
        ...
        self.cross_mamba = Mamba(dim, bimamba_type="v3") # , bimamba_type="v3"
        ...
        # self.conv = BasicConv2d(dim, dim, 1)
        self.norm1 = LayerNorm(dim, 'with_bias')
        self.norm2 = LayerNorm(dim, 'with_bias')
        self.dsconv_x = Conv2d_BN(dim, dim, ks=3, stride=1, pad=1, dilation=1,
                                groups=dim, norm_cfg=dict(type='BN2d', requires_grad=True))
        self.dsconv_y = Conv2d_BN(dim, dim, ks=3, stride=1, pad=1, dilation=1,
                                groups=dim, norm_cfg=dict(type='BN2d', requires_grad=True))
    ...
    def forward(self, x1, x2):
        ...
        # ms_resi = ms+ms_resi
        x_norm = self.norm1(x_fuse1)
        y_norm = self.norm2(x_fuse2)
        global_x,  global_y = self.cross_mamba(x_norm, extra_emb=y_norm)
        
        x_mamba = global_x.transpose(1, 2).view(B, C, H, W)
        y_mamba = global_y.transpose(1, 2).view(B, C, H, W)

        out_x1 = self.dsconv_x(x_mamba) + x_fuse1
        out_x2 = self.dsconv_y(y_mamba) + x_fuse2
        # out = self.conv(hsi + rgb)
...
class Interaction_hybrid_minxer(nn.Module):
    def __init__(self, dim, kernel_size=3, num_groups=2, chunk_numbers=[8,16], norm_layer=nn.LayerNorm, **kwargs):
        ...
        self.global_unit = CrossMamba(dim//2)
...
class MLLANet(nn.Module): # 1020
    def __init__(self, num_classes=3, binary_classes=1, normal_init=True, pretrained=False):
        ...
        self.interaction3 = Interaction_multi_granularity(96, kernel_size=7) 
        self.interaction2 = Interaction_multi_granularity(32, kernel_size=7) 
        self.interaction1 = Interaction_multi_granularity(24, kernel_size=7)
        self.interaction0 = Interaction_multi_granularity(16, kernel_size=7)
        ...
        self.fusion2 = Fusion(32,64,reduction=False)
    ...
    def forward(self, imgs, labels=None, return_aux=True):
        ...
        c0, c1, c2, c3, c4 = self.model(img1)
        c0_img2, c1_img2, c2_img2, c3_img2, c4_img2 = self.model(img2)
        
        cur1_3, cur2_3 = self.interaction3(c3, c3_img2) # 192, 16 , 16 -> 64, 16 , 16
        cur1_2, cur2_2 = self.interaction2(c2, c2_img2) # 80, 32 , 32 -> 32, 32 , 32
        cur1_1, cur2_1 = self.interaction1(c1, c1_img2) # 48, 64 , 64 -> 24, 64 , 64
        cur1_0, cur2_0 = self.interaction0(c0, c0_img2) # 16, 128 , 128 -> 8, 128 , 128
        ...
        if return_aux:
            ...
            #     draw_features(4,4,(F.interpolate(c1, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c1.png".format(savepath))
            #     draw_features(4,4,(F.interpolate(c3, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c3.png".format(savepath))
            #     draw_features(6,6,(F.interpolate(c5, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c5.png".format(savepath))
            #     draw_features(10,10,(F.interpolate(c7, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c7.png".format(savepath))

                
            #     draw_features(4,4,(F.interpolate(c1_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c1_img2.png".format(savepath))
            #     draw_features(4,4,(F.interpolate(c3_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c3_img2.png".format(savepath))
            #     draw_features(6,6,(F.interpolate(c5_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c5_img2.png".format(savepath))
            #     draw_features(10,10,(F.interpolate(c7_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/c7_img2.png".format(savepath))

            #     # cur1_3, cur2_3
                
            #     draw_features(10,10,(F.interpolate(cur1_3, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/cur1_3.png".format(savepath))
            #     draw_features(6,6,(F.interpolate(cur1_2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/cur1_2.png".format(savepath))
            #     draw_features(4,4,(F.interpolate(cur1_1, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/cur1_1.png".format(savepath))
            #     draw_features(4,4,(F.interpolate(cur1_0, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/cur1_0.png".format(savepath))

                
            #     draw_features(10,10,(F.interpolate(cur2_3, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/cur2_3.png".format(savepath))
            #     draw_features(6,6,(F.interpolate(cur2_2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/cur2_2.png".format(savepath))
            #     draw_features(4,4,(F.interpolate(cur2_1, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/cur2_1.png".format(savepath))
            #     draw_features(4,4,(F.interpolate(cur2_0, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/cur2_0.png".format(savepath))

            #     # encoder_outputs[2]
                
            #     draw_features(10,10,(F.interpolate(encoder_outputs[2], scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/encoder_outputs[2].png".format(savepath))
            #     draw_features(8,8,(F.interpolate(encoder_outputs[1], scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/encoder_outputs[1].png".format(savepath))
            #     draw_features(4,8,(F.interpolate(encoder_outputs[0], scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/encoder_outputs[0].png".format(savepath))


            #     draw_features(8,8,(F.interpolate(out2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/sequence_out2.png".format(savepath))
            #     draw_features(8,8,(F.interpolate(out3, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/sequence_out3.png".format(savepath))
            #     draw_features(4,8,(F.interpolate(out4, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/sequence_out4.png".format(savepath))


            #     draw_features(1,3,(F.interpolate(output, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/output.png".format(savepath))
            #     draw_features(1,3,(F.interpolate(output_middle1, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/output_middle1.png".format(savepath))
            #     draw_features(1,3,(F.interpolate(output_middle2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/output_middle2.png".format(savepath))

            #     draw_features(1,1,(F.interpolate(pred_v, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/pred_v.png".format(savepath))
            #     draw_features(1,1,(F.interpolate(output_binary, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/output_binary.png".format(savepath))
            #     draw_features(1,1,(F.interpolate(pred_v_binary, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/pred_v_binary.png".format(savepath))

            return output, output_middle1, output_middle2, pred_v, output_binary, pred_v_binary
