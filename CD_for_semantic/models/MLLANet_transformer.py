...
class Interaction_attention_various_head(torch.nn.Module):
    ...
    def forward(self, x1, x2, label=None):  
        ...
        k_1 = self.to_k_1(x1)
        k_2 = self.to_k_2(x2)
        k =  k_1-k_2 # torch.abs(k_1-k_2) # B, nh_kd, H, W
        ...
        # torch.abs(xx_row_1_part1-xx_row_2_part1).add(torch.abs(xx_column_1_part1-xx_column_2_part1)).reshape(B, self.dh//2, H, W)
        xx_part2 = (xx_row_1_part2-xx_row_2_part2).add(xx_column_1_part2-xx_column_2_part2).reshape(B, self.dh//2, H, W)
        # torch.abs(xx_row_1_part2-xx_row_2_part2).add(torch.abs(xx_column_1_part2-xx_column_2_part2)).reshape(B, self.dh//2, H, W)
        xx = torch.cat([xx_part1,xx_part2],dim=1)
        xx = (v_1-v_2).add(xx) # torch.abs(v_1-v_2).add(xx)
        att = self.sigmoid(self.proj(xx))
...
class HybridTokenMixer(nn.Module): ### D-Mixer
    ...
    def forward(self, x1, x2, relative_pos_enc=None):
        ...
        x_out1_2, x_out2_2, _ = self.global_unit(x1_2, x2_2)
