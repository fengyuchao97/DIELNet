...
class CPAMDec_Mix_large(nn.Module):
    ...
    def forward(self,x1,y1,x2,y2,label=None):
        ...
        if label is not None:
            ...
            att = attention.permute(0,2,1).view(m_batchsize,K,width,height)
            loss_att = self.loss_generator(torch.mean(att,dim=1),label)
            return out1, out2, loss_att#, lat_loss
        else:
...
class CPAMEnc_large(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.cat((feat1, feat2, feat3, feat4, feat5), 2) #self.conv1(x).view(b,c,-1) # 
...
class TMEncoder(nn.Module):
    def __init__(self, in_ch, enc_chs=(64, 128), expansion=1):
        ...
        self.layer1 = nn.Sequential(
            ...
            #     enc_chs[0]*exps,
            # ),
            nn.Conv2d(enc_chs[0], enc_chs[0]*exps, 3, 1, 1 , bias=False),
        ...
        self.layer2 = nn.Sequential(
            ...
            #     enc_chs[1]*exps,
            # ),
            nn.Conv2d(enc_chs[0] * exps, enc_chs[1] * exps, 3, 1, 1, bias=False),
...
class TDANet_large_interact(nn.Module): 
    def __init__(self, num_classes=1, drop_rate=0., normal_init=True, pretrained=False, show_Feature_Maps=False):
        super(TDANet_large_interact, self).__init__()
        self.video_len = 8
        self.show_Feature_Maps = show_Feature_Maps
        enc_chs = (16,32)
        # enc_chs = (32,64)
        ...
        self.local_consrative2 = Local_Block(48, window_size=16)
        self.local_consrative1 = Local_Block(32, window_size=16)
    ...
    def generate_transition_video_tensor(self, frame1, frame2, num_frames=8):
        ...
        frame = rearrange(transition_video, "l n c h w -> n l c h w")
        return frame
    ...
    def forward(self, img1, img2, labels=None, return_aux=True):
        frames = self.pair_to_video(img1, img2) # b, 9, 3, 256, 256
...
class TDANet_3D_Mobile(nn.Module): 
    def __init__(self, num_classes=1, drop_rate=0., normal_init=True, pretrained=False, show_Feature_Maps=False):
        super(TDANet_3D_Mobile, self).__init__()
        self.video_len = 8
        self.show_Feature_Maps = show_Feature_Maps
        enc_chs = (16,32)
        # enc_chs = (32,64)
        ...
        self.global_consrative2 = ContrastiveAtt_Block(96) # Local_Block(192)# 
        self.global_consrative1 = ContrastiveAtt_Block(32) # Local_Block(80)# 
        self.local_consrative2 = Local_Block(24, window_size=16)
        self.local_consrative1 = Local_Block(16, window_size=16)
        ...
        self.decoder1 = DecBlock(256, 128, num_classes) 
        self.decoder2 = DecBlock(192, 64, num_classes)
        self.decoder3 = DecBlock(64+16, 32, num_classes)
    ...
    def forward(self, imgs1, imgs2, labels=None, return_aux=True):
        ...
        mobile_img1 = self.model(imgs1)
        mobile_img2 = self.model(imgs2)

        c1,c2,c3,c4 = mobile_img1[0], mobile_img1[1], mobile_img1[2], mobile_img1[3]
        c1_img2,c2_img2,c3_img2,c4_img2 = mobile_img2[0], mobile_img2[1], mobile_img2[2], mobile_img2[3]


        if labels is not None:
            cur1_0, cur2_0, loss_local_att1 = self.local_consrative1(c1, c1_img2, labels) # 16, 128 , 128 -> 8, 128 , 128
            cur1_1, cur2_1, loss_local_att2 = self.local_consrative2(c2, c2_img2, labels) # 48, 64 , 64 -> 24, 64 , 64
            cur1_2, cur2_2, loss_att2 = self.global_consrative1(c3, c3_img2, labels) # 80, 32 , 32 -> 32, 32 , 32
            cur1_3, cur2_3, loss_att1 = self.global_consrative2(c4, c4_img2, labels) # 192, 16 , 16 -> 64, 16 , 16
            loss_att = loss_att1 + loss_att2 + 0.4*(loss_local_att1 + loss_local_att2)
        else:
            cur1_0, cur2_0 = self.local_consrative1(c1, c1_img2) # 16, 128 , 128 -> 8, 128 , 128
            cur1_1, cur2_1 = self.local_consrative2(c2, c2_img2) # 48, 64 , 64 -> 24, 64 , 64
            cur1_2, cur2_2 = self.global_consrative1(c3, c3_img2) # 80, 32 , 32 -> 32, 32 , 32
            cur1_3, cur2_3 = self.global_consrative2(c4, c4_img2) # 192, 16 , 16 -> 64, 16 , 16
...
class TDANet_3D_ResNet(nn.Module): 
    def __init__(self, num_classes=1, drop_rate=0., normal_init=True, pretrained=False, show_Feature_Maps=False):
        super(TDANet_3D_ResNet, self).__init__()
        self.video_len = 8
        self.show_Feature_Maps = show_Feature_Maps
        enc_chs = (16,32)
        # enc_chs = (32,64)
        ...
        self.global_consrative2 = ContrastiveAtt_Block(256) # Local_Block(192)# 
        self.global_consrative1 = ContrastiveAtt_Block(128) # Local_Block(80)# 
        self.local_consrative2 = Local_Block(64, window_size=16)
        self.local_consrative1 = Local_Block(64, window_size=16)
        ...
        self.fusion3 = Fusion(256,128,reduction=False) #Difference
        self.fusion2 = Fusion(128,64,reduction=False)
        self.fusion1 = Fusion(64,32,reduction=False)
        self.fusion0 = Fusion(64,16,reduction=False)
    ...
    def forward(self, imgs1, imgs2, labels=None, return_aux=True):
        ...
        if labels is not None:
            cur1_0, cur2_0, loss_local_att1 = self.local_consrative1(c0, c0_img2, labels) # 16, 128 , 128 -> 8, 128 , 128
            cur1_1, cur2_1, loss_local_att2 = self.local_consrative2(c1, c1_img2, labels) # 48, 64 , 64 -> 24, 64 , 64
            cur1_2, cur2_2, loss_att2 = self.global_consrative1(c2, c2_img2, labels) # 80, 32 , 32 -> 32, 32 , 32
            cur1_3, cur2_3, loss_att1 = self.global_consrative2(c3, c3_img2, labels) # 192, 16 , 16 -> 64, 16 , 16
            loss_att = loss_att1 + loss_att2 + 0.4*(loss_local_att1 + loss_local_att2)
        else:
            cur1_0, cur2_0 = self.local_consrative1(c0, c0_img2) # 16, 128 , 128 -> 8, 128 , 128
            cur1_1, cur2_1 = self.local_consrative2(c1, c1_img2) # 48, 64 , 64 -> 24, 64 , 64
            cur1_2, cur2_2 = self.global_consrative1(c2, c2_img2) # 80, 32 , 32 -> 32, 32 , 32
            cur1_3, cur2_3 = self.global_consrative2(c3, c3_img2) # 192, 16 , 16 -> 64, 16 , 16
...
class VideoNet(nn.Module): 
    def __init__(self, num_classes=1, drop_rate=0., normal_init=True, pretrained=False, show_Feature_Maps=False):
        ...
        self.decoder2 = DecBlock(96, 32, num_classes)
    ...
    def forward(self, imgs1, imgs2, labels=None, return_aux=True):
        ...
        cat2 = feats_v[1] # torch.cat([fuse2,],1) 
        cat1 = feats_v[0] #torch.cat([fuse1,],1) 

        dec2,output = self.decoder2(cat1,cat2)
