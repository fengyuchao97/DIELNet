...
class TransFuse_WSelf(nn.Module):
    def __init__(self,num_classes=3, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        # self.convdown=Conv(1024,1024,2,2,relu=False)
        ...
        self.final_x_binary = nn.Sequential(
            Conv(64, 64, 3, bn=True, relu=True),
            Conv(64, 1, 3, bn=False, relu=False)
        )
        
        # self.final_x_middle = nn.Sequential(
        #     Conv(128, 64, 3, bn=True, relu=True),
        #     Conv(64, num_classes, 3, bn=False, relu=False)
        #     )

        # self.final_1_middle = nn.Sequential(
        #     Conv(128, 64, 3, bn=True, relu=True),
        #     Conv(64, num_classes, 3, bn=False, relu=False)
        #     )

        # self.final_2_middle = nn.Sequential(
        #     Conv(128, 64, 3, bn=True, relu=True),
        #     Conv(64, num_classes, 3, bn=False, relu=False)
        #     )
    ...
    def forward(self, imgs1, imgs2, labels=None):
        ...
        map_x = self.final_2(finalx)
        map_1 = self.final_1(final1)
        map_2 = self.final_x(final2)

        map_x_binary = torch.sigmoid(self.final_x_binary(finalx))

        # final2_middle = self.upsamplex8(torch.abs(out3_g-out3_g_img2))
        # final1_middle = self.upsamplex8(torch.abs(out3_l-out3_l_img2))
        # finalx_middle = self.upsamplex8(torch.abs(x_up_2-x_up_2_img2))

        # map_2_middle = self.final_2_middle(final2_middle)
        # map_1_middle = self.final_1_middle(final1_middle)
        # map_x_middle = self.final_x_middle(finalx_middle)

        # if self.show_Feature_Maps:
        #     savepath=r'temp'

        #     draw_features(8,8,(F.interpolate(cross_2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_cross4_2_img1.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(curg_2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curg4_2_img1.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(curl_2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curl4_2_img1.png".format(savepath))

        #     draw_features(8,8,(F.interpolate(cross_2_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_cross4_2_img2.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(curg_2_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curg4_2_img2.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(curl_2_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curl4_2_img2.png".format(savepath))

        #     draw_features(8,8,(F.interpolate(cross_3, scale_factor=2, mode='bilinear')).cpu().detach().numpy(),"{}/Out_cross_3_img1.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(curg_3, scale_factor=2, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curg_3_img1.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(curl_3, scale_factor=2, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curl_3_img1.png".format(savepath))

        #     draw_features(8,8,(F.interpolate(cross_3_img2, scale_factor=2, mode='bilinear')).cpu().detach().numpy(),"{}/Out_cross_3_img2.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(curg_3_img2, scale_factor=2, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curg_3_img2.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(curl_3_img2, scale_factor=2, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curl_3_img2.png".format(savepath))

        #     draw_features(4,8,(F.interpolate(cross_4, scale_factor=1, mode='bilinear')).cpu().detach().numpy(),"{}/Out_cross_4_img1.png".format(savepath))
        #     draw_features(4,8,(F.interpolate(curg_4, scale_factor=1, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curg_4_img1.png".format(savepath))
        #     draw_features(4,8,(F.interpolate(curl_4, scale_factor=1, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curl_4_img1.png".format(savepath))

        #     draw_features(4,8,(F.interpolate(cross_4_img2, scale_factor=1, mode='bilinear')).cpu().detach().numpy(),"{}/Out_cross_4_img2.png".format(savepath))
        #     draw_features(4,8,(F.interpolate(curg_4_img2, scale_factor=1, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curg_4_img2.png".format(savepath))
        #     draw_features(4,8,(F.interpolate(curl_4_img2, scale_factor=1, mode='bilinear')).cpu().detach().numpy(),"{}/Out_curl_4_img2.png".format(savepath))

        #     draw_features(8,8,(F.interpolate(x_up_3, scale_factor=8, mode='bilinear')).cpu().detach().numpy(),"{}/Out_Merge_4_img1.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(x_up_3_img2, scale_factor=8, mode='bilinear')).cpu().detach().numpy(),"{}/Out_Merge_4_img2.png".format(savepath))

        #     draw_features(8,8,(F.interpolate(out2_l, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_MCNN_4_img1.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(out2_l_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_MCNN_4_img2.png".format(savepath))

        #     draw_features(8,8,(F.interpolate(out2_g, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_MTrs_4_img1.png".format(savepath))
        #     draw_features(8,8,(F.interpolate(out2_g_img2, scale_factor=4, mode='bilinear')).cpu().detach().numpy(),"{}/Out_MTrs_4_img2.png".format(savepath))

        #     draw_features(8,8,(final1).cpu().detach().numpy(),"{}/Out_CNN.png".format(savepath))
        #     draw_features(8,8,(final2).cpu().detach().numpy(),"{}/Out_Trs.png".format(savepath))
        #     draw_features(8,8,(finalx).cpu().detach().numpy(),"{}/Out_Merge.png".format(savepath))

        #     draw_features(1,1,(map_1).cpu().detach().numpy(),"{}/Final_CNN.png".format(savepath))
        #     draw_features(1,1,(map_x).cpu().detach().numpy(),"{}/Final_Trs.png".format(savepath))
        #     draw_features(1,1,(map_2).cpu().detach().numpy(),"{}/Final_Merge.png".format(savepath))
        return map_x, map_1, map_2, map_x_binary #, map_x_middle, map_1_middle, map_2_middle
    ...
    def init_weights(self):
        ...
        # self.final_x_middle.apply(init_weights)
        # self.final_1_middle.apply(init_weights)
        # self.final_2_middle.apply(init_weights)
...
class decoder(nn.Module):
    def __init__(self, drop_rate=0.2,):
        ...
        self.cross2 = MultiHeadCrossAttention(256, 320,ch_out=320, drop_rate=drop_rate/2,qkv_bias=True)
        self.cross3 = MultiHeadCrossAttention(128, 128,ch_out=128, drop_rate=drop_rate/2,qkv_bias=True)
        self.cross4 = MultiHeadCrossAttention(64, 64,ch_out=64, drop_rate=drop_rate/2,qkv_bias=True)
