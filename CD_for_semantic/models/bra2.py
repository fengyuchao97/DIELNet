...
class BiLevelRoutingCrossAttention(nn.Module):
    ...
    def forward(self, x1, x2, ret_attn_mask=False):
        ...
        kv_pix = self.kv_down(rearrange(kv2, 'n p2 h w c -> (n p2) c h w'))
        ...
        lepe = self.lepe(rearrange(kv2[..., self.qk_dim:], 'n (j i) h w c -> n c (j h) (i w)', j=self.n_win,
                                   i=self.n_win).contiguous())
        lepe = rearrange(lepe, 'n c (j h) (i w) -> n (j h) (i w) c', j=self.n_win, i=self.n_win)

        ############ gather q dependent k/v #################
        r_weight, r_idx = self.router(q1_win, k2_win)  # both are (n, p^2, topk)
        ...
        attn_weight = (q1_pix * self.scale) @ k_pix_sel  # (n*p^2, m, w^2, c) @ (n*p^2, m, c, topk*h_kv*w_kv) -> (n*p^2, m, w^2, topk*h_kv*w_kv)
        attn_weight = self.attn_act(attn_weight)
        out = attn_weight @ v_pix_sel  # (n*p^2, m, w^2, topk*h_kv*w_kv) @ (n*p^2, m, topk*h_kv*w_kv, c) -> (n*p^2, m, w^2, c) 矩阵乘法
