...
class FresUNet(nn.Module):
    ...
    def __init__(self, input_nbr=2*3, label_nbr=3):
        ...
        self.coupling_binary = nn.Conv2d(cur_depth + d1, 1, kernel_size=1)
...
#         pad5 = ReplicationPad2d((0, x53.size(3) - x5d.size(3), 0, x53.size(2) - x5d.size(2)))
        ...
        out = self.coupling(torch.cat((x, x1), 1))
        out_binary = torch.sigmoid(self.coupling_binary(torch.cat((x, x1), 1)))
        # x = self.sm(x)
