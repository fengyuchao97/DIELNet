...
class SimpleDecoder(nn.Module):
    def __init__(self, itm_ch, enc_chs, dec_chs):
        ...
        self.conv_out2 = Conv1x1(dec_chs[-1], 1)
...
class P2VNet(nn.Module):
    def __init__(self, in_ch=3, video_len=8, enc_chs_p=(32,64,128), enc_chs_v=(64,128), dec_chs=(256,128,64,32)):
        ...
        self.conv_out_v = Conv1x1(enc_chs_v[-1], 3)
    ...
    def forward(self, t1, t2, return_aux=True):
        ...
        pred, pred_binary = self.decoder(feats_p[-1], feats_p)
        ...
        if return_aux:
            ...
            return pred, pred_v, pred_binary
        else:
...
if __name__ == '__main__':
    ...
    input2 = torch.randn(2, 3, 256, 256).cuda()
    model = P2VNet().cuda()
